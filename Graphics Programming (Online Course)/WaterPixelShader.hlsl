...
float4 main(VertexToPixel input) : SV_TARGET
{
	...
	normalMap2 = normalTexure.Sample(basicSampler, input.uv2);
	...
	refractUV.x = input.refractionPosition.x / input.position.w / 2.0f + 0.5f;
	refractUV.y = -input.refractionPosition.y / input.position.w / 2.0f + 0.5f;

	// calcualte projected reflection texture coodinates
	reflectUV.x = input.reflectionPosition.x / input.position.w / 2.0f + 0.5f;
	reflectUV.y = input.reflectionPosition.y / input.position.w / 2.0f + 0.5f;

	// repostion the texture coordinates sampling position by the scaled normal map value to simulate the rippling wave effect
	reflectUV = reflectUV + (normal.xy * reflectRefractScale);
	refractUV = refractUV + (normal.xy * reflectRefractScale);

	// sample texture pixels from texture using the updated texture coordinates
	reflectionColor = reflectionTexture.Sample(basicSampler, reflectUV);
	refractionColor = refractionTexure.Sample(basicSampler, refractUV);
	...
	if (specular > 1.0f)
	{
		specular = pow(specualar, specularShinniness);
